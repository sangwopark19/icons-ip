-- Keep community reaction counts aligned with viewer visibility and blocks.

drop function if exists public.community_post_reaction_counts(uuid[]);
drop function if exists public.community_post_reaction_counts(uuid[], uuid[]);
...
as $$
  ...
  comment_counts as (
    ...
    where not (comments.user_id = any(blocked_user_ids))
