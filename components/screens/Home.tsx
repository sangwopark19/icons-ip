    ...
    <header style={{ position: 'relative', paddingTop: 'calc(var(--nav-h) + 40px)', overflow: 'hidden' }}>
      ...
      <div className="wrap" style={{ position: 'relative', zIndex: 2, textAlign: 'center', paddingBottom: 64 }}>
        ...
        <div className="rise" style={{ animationDelay: '.22s', display: 'flex', gap: 12, justifyContent: 'center', marginTop: 32, flexWrap: 'wrap' }}>
          <button className="btn btn-holo" onClick={() => go('binder')}>지금 시작하기 <Icon name="arrow" size={17} /></button>
          <button className="btn btn-ghost" onClick={() => go('shop')}>굿즈샵 둘러보기</button>
        </div>
...
function VerticalCard({ ip, go }: { ip: Ip; go: Go }) {
  return (
    <button className="card lift" onClick={() => go('ip', ip.id)} style={{ padding: 0, overflow: 'hidden', textAlign: 'left', position: 'relative', cursor: 'pointer' }}>
...
function EventStrip({ go }: { go: Go }) {
  return (
    <div style={{ display: 'flex', gap: 18, overflowX: 'auto', paddingBottom: 8, scrollbarWidth: 'none' }}>
      {DATA.EVENTS.map((e) => (
        <button key={e.id} className="card lift" onClick={() => go('events')} style={{ flex: '0 0 320px', padding: 0, overflow: 'hidden', textAlign: 'left', cursor: 'pointer' }}>
      ...
      <section className="section">
        <div className="wrap">
          ...
          <div className="grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))' }}>
            {featured.map((ip) => <VerticalCard key={ip.id} ip={ip} go={go} />)}
          </div>
        </div>
      </section>
      ...
      <section className="section" style={{ background: 'var(--bg-2)', borderTop: '1px solid var(--line)', borderBottom: '1px solid var(--line)' }}>
        <div className="wrap">
          ...
          <div className="grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))' }}>
            {DATA.GOODS.slice(0, 8).map((g) => <GoodsCard key={g.id} g={g} go={go} />)}
          </div>
        </div>
      </section>
          ...
          <div className="grid" style={{ gridTemplateColumns: 'minmax(280px, 0.9fr) 1.1fr', alignItems: 'center', gap: 48 }}>
            <div>
              ...
              <div className="row" style={{ marginTop: 28, gap: 12, flexWrap: 'wrap' }}>
                <button className="btn btn-holo" onClick={() => go('binder')}>내 수집 바인더 <Icon name="card" size={16} /></button>
                <button className="btn btn-ghost" onClick={() => go('exchange')}>카드 교환 마켓</button>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 16, justifyContent: 'center' }}>
              {[DATA.CARDS[0], DATA.CARDS[2], DATA.CARDS[8]].map((c, i) => (
                <div key={c.id} style={{ marginTop: i === 1 ? -28 : i === 2 ? 18 : 0 }}>
                  <Collectible card={{ ...c, owned: true }} onClick={() => go('binder')} />
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="wrap">
          <div className="grid" style={{ gridTemplateColumns: '0.85fr 1.15fr', gap: 44, alignItems: 'start' }}>
            <div style={{ position: 'sticky', top: 96 }}>
              ...
              <button className="btn btn-primary" style={{ marginTop: 24 }} onClick={() => go('community')}>커뮤니티 참여하기 <Icon name="arrow" size={16} /></button>
            ...
            <div className="col" style={{ gap: 14 }}>
              {DATA.POSTS.map((p) => <FeedPreview key={p.id} p={p} go={go} />)}
            </div>
          </div>
        </div>
      </section>
