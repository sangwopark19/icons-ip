...
import { DATA, type Good, type Ip, type Stock } from '@/lib/data';
import { Poster } from './Poster';

const STOCK_LABEL: Record<Stock, string | null> = { low: '품절임박', soldout: '품절', ok: null };

export function GoodsCard({ g, href = '/shop', ip }: { g: Good; href?: string; ip?: Pick<Ip, 'title' | 'glyph'> | null }) {
  const displayIp = ip ?? DATA.ipById(g.ip);
  ...
  return (
    <Link className="card lift" href={href} style={{ display: 'block', padding: 0, overflow: 'hidden', textAlign: 'left', cursor: 'pointer' }}>
      <Poster bg={g.img} glyph={displayIp?.glyph} showGlyph={false} ratio="1 / 1" radius={0}>
      ...
      <div style={{ padding: '14px 15px 16px' }}>
        <div className="mono" style={{ fontSize: 11, color: 'var(--faint)' }}>{displayIp?.title} · {g.type}</div>
