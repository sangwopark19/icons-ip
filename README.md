...
Production 확인 메일의 redirect URL은 allow-list와 정확히 맞도록 query 없는 `/auth/callback`만 사용한다. 보호 액션에서 시작한 `next` 경로는 같은 브라우저에 한해 짧은 httpOnly 쿠키로 보존하고, 콜백에서 세션 쿠키를 설정한 뒤 온보딩 완료 여부에 따라 `/onboarding` 또는 보존된 `next` 경로로 이동한다. `main` 배포 workflow는 `SUPABASE_ACCESS_TOKEN`으로 위 Site URL과 Redirect URLs를 확인하고, 누락 시 동일 값으로 보정한다.
